...
importSSN_stream_past <- function(shreve_obj, location="Miho", multipleplaces=FALSE, RID="original"){
  ...
  edges  <- readOGR("Data/KRF_3.0_Geumgang/KRF_ver3_LINE_금강수계.shp")
  # edges  <- st_read("Data/KRF_3.0_Geumgang/KRF_ver3_LINE_금강수계.shp")
  # rownames(edges@data) <- edges@data[, "RCH_ID"]
  # reach ID (print rid)
  
  # (b) monitoring sites data
  sites <- read.csv("Data/ProcessedData/금강장소(엑셀편집)UTF8.csv",header=T)
  ...
  alldata1 <- read.table("Data/ProcessedData/수질20082013.txt",sep=',', header=T)
  alldata2 <- read.table("Data/ProcessedData/수질20132018.txt",sep=',', header=T)
  alldata <- rbind(alldata1, alldata2)
  
  # (d) preprocessed data 
  if(location=="Miho"){
    data <- readRDS("Data/ProcessedData/Geum(miho).RDS")
    data2 <- readRDS("Data/ProcessedData/Geum.RDS")
  }else if(location=="Full"){
    data <- readRDS("Data/ProcessedData/Geum.RDS")
  }else{
    stop("Wrong location")
  }
  shape <- data$shape
  ...
  if(location=="Miho"){
    # flow_network_original <- line2network(path = "Data/ProcessedData/", layer="GeumMiho", tolerance =  0.000001, reproject ="+proj=longlat +datum=WGS84", supplyprojection = NULL)
    # flow_network <- line2network(path = "Data/ProcessedData/", layer="thinGeumMiho", tolerance =  0.000001, reproject ="+proj=longlat +datum=WGS84", supplyprojection = NULL)
    flow_network_original <- line2network(path = "Data/ProcessedData/", layer="GeumMiho", tolerance =  0.000001, reproject ="+proj=longlat +datum=WGS84")
    flow_network <- line2network(path = "Data/ProcessedData/", layer="thinGeumMiho", tolerance =  0.000001, reproject ="+proj=longlat +datum=WGS84")
    # plot(flow_network)
  }else if(location=="Full"){
    # flow_network_original <- line2network(path = "Data/ProcessedData/", layer="Geum", tolerance =  0.000001, reproject ="+proj=longlat +datum=WGS84", supplyprojection = NULL)
    # flow_network <- line2network(path = "Data/ProcessedData/", layer="thinGeum", tolerance =  0.000001, reproject ="+proj=longlat +datum=WGS84", supplyprojection = NULL)
    flow_network_original <- line2network(path = "Data/ProcessedData/", layer="Geum", tolerance =  0.000001, reproject ="+proj=longlat +datum=WGS84")
    flow_network <- line2network(path = "Data/ProcessedData/", layer="thinGeum", tolerance =  0.000001, reproject ="+proj=longlat +datum=WGS84")
    # plot(flow_network)
  }else{
    stop("Wrong location")
  }
  ...
  if(location=="Miho"){
    ...
    shape_tinned <- readOGR("Data/ProcessedData/thinGeumMiho.shp")
    # shape_tinned <- st_read("Data/ProcessedData/thinGeumMiho.shp")
  }else if(location=="Full"){
    # upDist should also be calculated
    DistanceUpstream_seg <- rep(0, nrow(shape@data))
    for(ii in 1:length(DistanceUpstream_seg)){
    ...
    shape_tinned <- readOGR("Data/ProcessedData/thinGeum.shp")
  }
  ...
  shapedata <- cbind(shapedata, rid=c(1:nrow(shape@data)), upDist=DistanceUpstream_seg, Length=shape@data$Shape_Leng, netID=rep(1,nrow(shape@data)), shreve=shreve_obj[match(shapedata$OBJECTID, data2$shape$OBJECTID)] )
